module Nfse
  module Fortaleza
    class Services::ConsultarNfseRps
      module Result
        class Success
          ...
          def initialize(body:)
            ...
            @notas_fiscais = body.dig("ns3:ConsultarNfseRpsResposta", "ListaNfse", "CompNfse").map do |nfse|
              NotaFiscal.new(nfse)
            end
          end
        ...
        class Failure
          ...
          def initialize(body:)
            ...
            result = body.dig("ns3:ConsultarNfseRpsResposta", "ListaMensagemRetorno", "MensagemRetorno")
      ...
      def initialize(rps:, certificate:, private_key:)
        @rps = rps
      ...
      def self.call(rps:, certificate:, private_key:)
        new(rps:, certificate:, private_key:).call
      end

      def call
        client = Wsdl.client(certificate: certificate, private_key: private_key)

        response = client.call(:consultar_nfse_por_rps_v3, message: {
          "Cabecalho!" => cabecalho_xml,
          "ConsultarNfseRpsEnvio!" => data_xml # NOTE: the "!" character tells Gyoku to not escape the content.
        })
        ...
        Result.load(
          body: ::Nori.new.parse(
            response.body.dig(:consultar_nfse_por_rps_v3_response, :consultar_nfse_rps_resposta)
          )
        )
      end
      ...
      attr_reader :rps, :certificate, :private_key
        ...
        parsed_data = ::Gyoku.xml(
          {
            ...
            "Prestador" => {
              ...
              "InscricaoMunicipal" => rps.prestador.inscricao_municipal
            }
          }
        )
